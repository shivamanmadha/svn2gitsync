 ...
 */
public class ICEImpactFullMarketDataHandler {
	...
	private ICEFullBookHandler bookHandler = null;
	private MarketDataNotifier mdNotifier = null;


	/**
	 * ICEMarketDataHandler constructor
	 */
	public ICEImpactFullMarketDataHandler(MarketDataNotifier mdn)
	{
		mdNotifier = mdn;
	}
	...
	public ArrayList getBidBook(String symbol)
	{
		if(bookHandler != null)
		{
			return bookHandler.getBidBook(symbol);
		}
	...
	public ArrayList getOfferBook(String symbol)
	{
		if(bookHandler != null)
		{
			return bookHandler.getOfferBook(symbol);
		}
    ...
    private void initializeData( String symbol )
    {
		...
		if(bookHandler == null)
		{
			bookHandler = new ICEFullBookHandler(mdNotifier);
		}
		else
		{
			bookHandler.clearBidBook(symbol);
			bookHandler.clearOfferBook(symbol);
		}
		    				...
		    				String price)
    {
    	...
    	mdNotifier.openPrice(symbol,price,true);
    }
    /**
		    				...
		    				String price)
    {
    	...
    	mdNotifier.settlementPrice(symbol,price,true);
    }
    /**
		    				...
		    				String price)
    {
    	...
    	mdNotifier.highPrice(symbol,price,true);
    }
    /**
		    				...
		    				String price)
    {
    	...
    	mdNotifier.lowPrice(symbol,price,true);
    }
    /**
		    				...
		    				String price)
    {
    	...
    	mdNotifier.vwapPrice(symbol,price,true);
    }
    /**
		    				...
		    				double size)
    {
    	...
    	mdNotifier.openInterest(symbol,size,true);
    }

	public void processDeleteOrder(String symbol,DeleteOrderMessage msg)
	{
  				String orderID = String.valueOf(msg.OrderID);
  				int sequenceID = 0;
  				double price = 0;
  				String dp = "0";
  				double size = 0;
    	...
    	if(!handleIncrRefreshBidDelete(symbol,orderID,sequenceID,size,dp,price))
			...
			// Do the following only if debugging
			//System.out.println("Processed - mdEType" + mdEType);
			//System.out.println("Bid");
 	 		//bookHandler.displayBidBook(symbol);
			//System.out.println("Offer");
 	 		//bookHandler.displayOfferBook(symbol);


	}

	public void processAddModifyOrder(String symbol,AddModifyOrderMessage msg)
	{
  				String orderID = String.valueOf(msg.OrderID);
  				int sequenceID = msg.OrderSeqID;
  				double price = msg.Price;
  				String dp = String.valueOf(msg.Price);
  				double size = msg.Quantity;
  				char side = msg.Side;
	...
	public void processTrade(String symbol,TradeMessage msg)
	{
  				String orderID = String.valueOf(msg.OrderID);
  				int sequenceID = 0;
  				double price = msg.Price;
	...
	public void processCancelledTrade(String symbol,CancelledTradeMessage msg)
	{
  				String orderID = String.valueOf(msg.OrderID);
  				int sequenceID = 0;
  				double price = msg.Price;
  				...
  				handleIncrRefreshTradeDelete(symbol,orderID,sequenceID,size,dp,price,null);
	...
	public void processMarketSnapshotOrder(String symbol,MarketSnapshotOrderMessage msg)
	{
  				String orderID = String.valueOf(msg.OrderID);
  				int sequenceID = msg.OrderSeqID;
  				double price = msg.Price;
	...
	public void processMarketSnapshot(String symbol,MarketSnapshotMessage msg)
	{
		initializeData(symbol);
  		...
  		handleFullRefreshOpeningPrice(symbol,String.valueOf(msg.OpeningPrice));
  		handleFullRefreshSettlementPrice(symbol,String.valueOf(msg.SettlementPrice));
  		handleFullRefreshHighPrice(symbol,String.valueOf(msg.High));
  		handleFullRefreshLowPrice(symbol,String.valueOf(msg.Low));
  		handleFullRefreshVWAPPrice(symbol,String.valueOf(msg.VWAP));
  		handleFullRefreshOpenInterest(symbol,msg.OpenInterest);
  		handleIncrRefreshVolumeNew(symbol,msg.TotalVolume);
  		// handle EFPVolume, BlockVolume, EFSVolume
	...
	public void processMarketStatistics(String symbol,MarketStatisticsMessage msg)
	{



  		handleIncrRefreshHighPriceNew(symbol,String.valueOf(msg.High));
  		handleIncrRefreshLowPriceNew(symbol,String.valueOf(msg.Low));
  		handleIncrRefreshVWAPPriceNew(symbol,String.valueOf(msg.VWAP));
  		handleIncrRefreshVolumeNew(symbol,msg.TotalVolume);
  		// handle EFPVolume, BlockVolume, EFSVolume

	}
	public void processOpenPrice(String symbol,OpenPriceMessage msg)
	{
  		handleIncrRefreshOpeningPriceNew(symbol,String.valueOf(msg.OpenPrice));
	}
	public void processOpenInterest(String symbol,OpenInterestMessage msg)
	{
  		handleIncrRefreshOpenInterestNew(symbol,msg.OpenInterest);
	}
	public void processSettlementPrice(String symbol,SettlementPriceMessage msg)
	{
  		handleIncrRefreshSettlementPriceNew(symbol,String.valueOf(msg.SettlementPrice));
	}
		...
		initializeData(symbol);
  		...
  		if( rgArray != null )
  		{
  			...
  			for (int i = 0; i < asize; i++) {
  				...
  				{
  					Logger.error("UNKNOWN MDETYPE = " + mdEType);
  				}
		...
		if(mdBids == null || mdOffers == null || mdTrades == null)
		{
  		...
  		if( rgArray != null )
  		{
  			...
  			for (int i = 0; i < asize; i++) {
				...
				// ignore block trades
				if(rg.getTradeType() != null && !rg.getTradeType().equals("6"))
				//if(rg.getTradeType() != null)
				{
  				...
  				*/
  				...
  				{
  					Logger.error("UNKNOWN MDUPDATE ACTION= " + mdUpdateAction);
  				}

				// Do the following only when debugging
    				...
    				double price)
    {
    	...
    	if(mdTrades.containsKey(symbol))
    	{
    		...
    		{
    			// IMPACT - no need to do sequenceID check
    	...
    	mdNotifier.lastTrade(symbol,
	    		     new MarketDataTrade(orderID,sequenceID,size,dp,price)
    				,false);


    	// we should also remove the appropriate order - either bid or offer
    	if(!handleIncrRefreshBidDelete(symbol,orderID,sequenceID,size,dp,price,true))
    		handleIncrRefreshOfferDelete(symbol,orderID,sequenceID,size,dp,price,true);
		    				...
		    				String price)
    {
    	...
    	{
    		((QuoteData)mdSummary.get(symbol)).openPrice = price;
    		mdNotifier.openPrice(symbol,price,false);
    	}
    	else
    	{
    		Logger.debug("ERROR - NO BOOK ENTRY FOR SYMBOL = " + symbol);
		    				...
		    				String price)
    {
    	...
    	{
    		((QuoteData)mdSummary.get(symbol)).settlementPrice = price;
    		mdNotifier.settlementPrice(symbol,price,false);
    	}
    	else
    	{
    		Logger.debug("ERROR - NO BOOK ENTRY FOR SYMBOL = " + symbol);
		    				...
		    				String price)
    {
    	...
    	{
    		((QuoteData)mdSummary.get(symbol)).highPrice = price;
    		mdNotifier.highPrice(symbol,price,false);
    	}
    	else
    	{
    		Logger.debug("ERROR - NO BOOK ENTRY FOR SYMBOL = " + symbol);
		    				...
		    				double size)
	{
    	if(mdSummary.containsKey(symbol))
    	{
    		...
    		mdNotifier.volume(symbol,size,false);
    	}
    	else
    	{
    		Logger.debug("ERROR - NO BOOK ENTRY FOR SYMBOL = " + symbol);
		    				...
		    				String price)
    {
    	...
    	{
    		((QuoteData)mdSummary.get(symbol)).lowPrice = price;
    		mdNotifier.lowPrice(symbol,price,false);
    	}
    	else
    	{
    		Logger.debug("ERROR - NO BOOK ENTRY FOR SYMBOL = " + symbol);
		    				...
		    				String price)
    {
    	...
    	{
    		((QuoteData)mdSummary.get(symbol)).vwapPrice = price;
    		mdNotifier.vwapPrice(symbol,price,false);
    	}
    	else
    	{
    		Logger.debug("ERROR - NO BOOK ENTRY FOR SYMBOL = " + symbol);
		    				...
		    				double size)
    {
    	...
    	{
    		((QuoteData)mdSummary.get(symbol)).openInterest = size;
    		mdNotifier.openInterest(symbol,size,false);
    	}
    	else
    	{
    		Logger.debug("ERROR - NO BOOK ENTRY FOR SYMBOL = " + symbol);
    				...
    				double price)
    {
    	if(mdBids.containsKey(symbol))
    	{
    		...
    		if(bl.containsKey(orderID))
    		{
    			...
    			{
    				bookHandler.removeBid(symbol,oldBid.price, oldBid.displayPrice, oldBid.size,true);
    				bookHandler.addBid(symbol,price, dp, size);
    			}
    			else
    			{
    				bookHandler.changeBid(symbol,price,dp,size,oldBid.size);
    			}
    				...
    				double price)
    {
    	if(mdOffers.containsKey(symbol))
    	{
    		...
    		if(ol.containsKey(orderID))
    		{
    			...
    			{
    				bookHandler.removeOffer(symbol,oldOffer.price, oldOffer.displayPrice, oldOffer.size, true);
    				bookHandler.addOffer(symbol,price, dp, size);
    			}
    			else
    			{
    				bookHandler.changeOffer(symbol,price,dp,size,oldOffer.size);
    			}
   ...
   /**
    ...
    private boolean handleIncrRefreshBidDelete(String symbol,
    				...
    				double price)
    {
    	return handleIncrRefreshBidDelete(symbol,orderID,sequenceID,size,dp,price,false);
    }
    /**
     * method to handle incr refresh bid delete
     * @param symbol
    				...
    				boolean fromNewTrade)
    {
    	...
    	if(mdBids.containsKey(symbol))
    	{
    		...
    		if(bl.containsKey(orderID))
    		{
    			...
    			// IMPACT spec does not give price and display price
    			price = bid.price;
    			dp = bid.displayPrice;
    	...
    	if(removed)
    	{
    		if(fromNewTrade)
    		{
    			//System.out.println((sizeRemoved!=size)?"****PARTIAL BID TRADE****":"****FULL BID TRADE****");
    			bookHandler.removeBid(symbol,price,dp,sizeRemoved,sizeRemoved!=size);
    		}
    		else
    ...
    private boolean handleIncrRefreshOfferDelete(String symbol,
    				...
    				double price)
    {
    	return handleIncrRefreshOfferDelete(symbol,orderID,sequenceID,size,dp,price,false);
    }
    /**
     * method to handle incr refresh offer delete
     * @param symbol
    				...
    				boolean fromNewTrade)
    {
    	...
    	if(mdOffers.containsKey(symbol))
    	{
    		...
    		if(ol.containsKey(orderID))
    		{
    			...
    			// IMPACT spec does not give price info
    			price = offer.price;
    			dp = offer.displayPrice;
    	...
    	if(removed)
    	{
    		if(fromNewTrade)
    		{
    			//System.out.println((sizeRemoved!=size)?"****PARTIAL OFFER TRADE****":"****FULL OFFER TRADE****");
    			bookHandler.removeOffer(symbol,price,dp,sizeRemoved,sizeRemoved!=size);
    		}
    		else
