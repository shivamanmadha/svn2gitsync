 ...
 */
public class FixToolsWindow extends JFrame implements LogonHandler,
					...
					
{
	...
	private JmsFixClientSession fcSession = null;	 
 ...
 */
public FixToolsWindow(String title) {
	super(title);
...
public void connectToFixServer() 
{
	...
	{	
		...
		statusPanel.addStatusMessage("About to Connect to Fix Server");
		...
		try
		{
			...
			if(mExchange.equals("ICE"))
			{
				if(pb)
				{
				if(enableSecuritiesSession)
					fcSession = new ICEJmsFixClientSession(pb);
				else if(enableMarketDataSession)
					fcSession = new ICEJmsMDFixClientSession(pb);
				else
					fcSession = new JmsFixClientSession(pb);					
				}
				else
				{
				if(enableSecuritiesSession)
					fcSession = new ICEJmsFixClientSession(serverHost,port);
				else if(enableMarketDataSession)
					fcSession = new ICEJmsMDFixClientSession(serverHost,port);
				else
					fcSession = new JmsFixClientSession(serverHost,port);
				}
			}
			else
			{
		...
		else
		{
			...
			fcSession.start();
			if(mExchange.equals("ICE") && enableMarketDataSession)
			{
				fcSession.sendResetLogonMessage();
			}
			else
			{
				fcSession.sendLogonMessage();
			}
			connected = true;
		}
	}
...
private void initializeConfig() 
{
	    try
	    {
	    	...
	    	//targetCompID = ConfigurationService.getValue(ConfigConstants.FIXCLIENT_TARGET_COMP_ID);
 		     ...
 		     if(mExchange.equals("ICE"))
 		     {
 		     	String strSec = ConfigurationService.getValue(ConfigConstants.FIXCLIENT_ENABLE_SECURITIES);
 		     	if(strSec.equals("Y")) enableSecuritiesSession = true;
 		     	String strMkt = ConfigurationService.getValue(ConfigConstants.FIXCLIENT_ENABLE_MKTDATA);
...
public static void main(String[] args) 
{
	...
	FixToolsWindow toolsWindow = new FixToolsWindow("FixToolsWindow");
	toolsWindow.pack();
...
public void resetSequenceNum(String newSeq)
{
	if( connected )
	{
		    //if(mExchange.equals("ICE"))
  		    //{
  		    //	if(newSeq.equals("100"))
			//		fcSession.sendTraderLogonMessage();  
			//	else
			//		fcSession.sendSecurityDefinitionRequest(newSeq);   		    		   
			//}   
   		    //else
   		    //{  
				 fcSession.sendSequenceReset(Integer.parseInt(newSeq));
   		    //}
   		    /*Test code 
   		    FixNews news = new FixNews();
   		    news.setHeadLine("Test Headline");
   		    news.setUrgency("0");
   		    news.setUserName("Test");
   		    news.setLinesOfText(1);
   		    FixRGLinesOfText_B rg = new FixRGLinesOfText_B();
   		    rg.setText("The ICE WTI Crude Futures markets will pre-open in 1 minute.");
   		    news.addRepeatingGroup(rg);
   		    fcSession.processApplicationMessage(news,news.getMsgType());
   		    */
					     ...
					     boolean isBuy)
{
		...
		ord.setClOrderID(fixOrderIdGen.getNextOrderId());		
		//ord.setSenderCompID(senderCompID);
		//ord.setTargetCompID(targetCompID);
		//Date date = new Date(0);
		//ord.setSendingTime(date);
		//ord.setTransactTime(date);
		
		ord.setOrdType(orderType);
		String fixSide = isBuy?"1":"2";
		ord.setSide(fixSide);
		ord.setTimeInForce(tif);
		ord.setSymbol(symbol);
		ord.setOrderQty(qty.toString());
		ord.setPrice(price);

	    ord.setHandlInst("1");
		
		if( fcSession != null && connected )
		{
			fcSession.sendOrder(ord);
		}
		else
		{
			displayError("Not Connected to Fix Serger","OrderEntry");
		}
}
