package com.exsys.codegen.impact;

import java.io.*;
import java.util.*;

public class ImpactMessageGenerator
{
    private int length = 0;
    public static String NON_NULL_IND = "NN";
    public ImpactMessageGenerator(String file, String name, String msgType )
    {
      ...
      try
      {
        ...
        writer.println( "package com.exsys.impact.message;\n");
        ...
        writer.println();
        writer.println("public class "+name+" extends ImpactMessage");
        writer.println("{");

        writer.print("\t");
        writer.println("public "+name+"()");
        writer.println("\t{");
        writer.println("\t\t super(ImpactMessageConstants.IMPACT_MESSAGE_LENGTH_"+msgType+",true);");
        
        writer.println("\t\t setMessageType(ImpactMessageConstants.IMPACT_MESSAGE_TYPE_"+msgType+");");
        
        writer.println("\t}");

        writer.print("\t");
		writer.println("public "+name+"(byte[] newBytes) throws ImpactProtocolError");
        writer.println("\t{");
        writer.println("\t\t super(newBytes);");
writer.println("\t\t setMessageType(ImpactMessageConstants.IMPACT_MESSAGE_TYPE_"+msgType+");");        
        writer.println("\t\t if(newBytes.length != ImpactMessageConstants.IMPACT_MESSAGE_LENGTH_"+msgType+")");
        writer.println("\t\t {");
        writer.println("\t\t\t throw new ImpactProtocolError(\"Invalid Message Length\",");
        writer.println("\t\t\t \"MessageLength for "+msgType+" Message = \"+");
        writer.println("\t\t\t ImpactMessageConstants.IMPACT_MESSAGE_LENGTH_"+msgType+");");
        writer.println("\t\t }");
    ...
    {
        ImpactMessageGenerator c = new ImpactMessageGenerator( args[0], args[1], args[2] );
    }
