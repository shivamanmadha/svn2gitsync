...
public class RLCBookWindow
  	extends RLCMarketDataBaseHandler
  	implements ListSelectionListener
{
  // ========================================================================
  // ========================================================================
  public RLCBookWindow() throws ConfigFileNotFound, Exception
  {
      //processConfig();
    super();
  ...
  private void processConfig()
  {
    try
    {
      ...
      mResponseSubjectMY = ConfigurationService.getValue("MYResponseSubject");
      mResponseSubjectMA = ConfigurationService.getValue("MAResponseSubject");
...
synchronized public void processMYMessage(RLCMYMessage rlcMsg)
{
	System.out.println("Book:processMYMessage");
...
synchronized public void processMAMessage(RLCMAMessage rlcMsg)
{
	System.out.println("Book:processMAMessage");
	//System.out.println(rlcMsg.toString());
	String instr = rlcMsg.getCompleteInstrumentCode();
	 ...
	 for( int i=0; i<5; i++)
	 {
		if(str.charAt(i) == '1')
		{
			bh.handleMABookRow(instr,
					i+1,
					Integer.valueOf(rlcMsg.getNumberOfBuyOrders(index)).intValue(),
					rlcMsg.getBuyLimitQuantityAsint(index),
					rlcMsg.getBuyLimitPriceAsPrice(index),
					Integer.valueOf(rlcMsg.getNumberOfSellOrders(index)).intValue(),
					rlcMsg.getSellLimitQuantityAsint(index),
					rlcMsg.getSellLimitPriceAsPrice(index)
					);
			index++;
...
public void processRestoredMAMessage(RLCMAMessage rlcMsg)
{
  ...
  public Component createResponseLog()
  {
    ...
    areaScrollPane.setBorder(
      BorderFactory.createCompoundBorder(
        BorderFactory.createCompoundBorder(
          BorderFactory.createTitledBorder("MAMYMessages Log"),
  ...
  public void displayMessage(String symbol)
  {
	  ...
	  RLCMYBookRow[] rows = bh.getBook(symbol);
	  if(rows != null)
	  {
  ...
  private void initializeSession()
  {
    try
    {
      ...
      mSessionMgr.receiveMarketDataMessages(this,mResponseSubjectMY);
      mSessionMgr.receiveMarketDataMessages(this,mResponseSubjectMA);
    }
    catch( Exception exc )
    {
      exc.printStackTrace();
    ...
    try
    {
      ...
      if(restore.equals("Y"))
      {
 	    ...
 	    for(int i=0; i<inList.size();i++)
	    {
 	     ...
 	     ArrayList inList1 = RLCFileManager.getRLCMessagesList(inFile,"MA");

 	     // then process in list
 	    for(int i=0; i<inList1.size();i++)
	    {
	    	System.out.println("Restoring");
    		RLCMAMessage msg = (RLCMAMessage)inList1.get(i);
  ...
  private String mResponseSubjectMY = null;
  private String mResponseSubjectMA = null;
